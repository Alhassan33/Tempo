...
export default function CollectionPage() {
  ...
  // ✅ FIX: never render blank screen — show clear error state if collection null
  if (!collection) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-5 px-6 text-center fade-up">
      <div className="text-5xl mb-2">🔍</div>
      <h1 className="text-2xl font-extrabold" style={{ color: "#e6edf3" }}>Collection Not Found</h1>
      <p className="text-sm" style={{ color: "#9da7b3" }}>
        No collection matched{" "}
        <code className="font-mono text-xs px-2 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.06)", color: "#00E6A8" }}>
          {id}
        </code>
      </p>
      <p className="text-xs" style={{ color: "#6b7280" }}>
        Check the URL — use the correct slug or a 0x contract address.
      </p>
      <div className="flex gap-3 justify-center flex-wrap mt-2">
        <a href="/studio"
          className="h-10 px-5 rounded-xl text-sm font-bold flex items-center gap-2"
          style={{ background: "rgba(0,230,168,0.1)", color: "#00E6A8", border: "1px solid rgba(0,230,168,0.25)", textDecoration: "none" }}>
          My Studio
        </a>
        <a href="/"
          className="h-10 px-5 rounded-xl text-sm font-bold flex items-center gap-2"
