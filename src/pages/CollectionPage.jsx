...
export default function CollectionPage() {
  ...
  // ✅ Working stats hook from Collection.txt
  ...
  // Active listings sorted cheapest first, hoisted to top of grid
  const activeListings = useMemo(() =>
    (listings || []).filter(l => l.active).sort((a, b) => Number(a.price) - Number(b.price)),
  [listings]);

  // Set of listed tokenIds so we don't double-render them
  ...
  // ✅ Stats from Collection.txt — uses rpcStats with collection fallback
  const stats = useMemo(() => {
  ...
  // ✅ Fetch unlisted tokens only (skip listed IDs)
  const fetchPage = useCallback(async (pageNum) => {
    ...
    const results = await Promise.all(ids.map(async (tokenId) => {
      ...
      if (listedIds.has(idStr)) return null; // skip listed — shown at top
  ...
  return (
    <div className="fade-up min-h-screen pb-20" style={{ background: "#0b0f14" }}>
      ...
      <div className="px-4 sm:px-6 max-w-7xl mx-auto -mt-16 relative z-10">
        ...
        {/* Stats Grid — same as Collection.txt */}
        ...
        {/* Items Tab — listed first, then unlisted */}
        {tab === "Items" && (
          <>
