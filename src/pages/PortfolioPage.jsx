...
function NFTCard({ nft, view, onList }) {
  ...
  // ✅ Fixed: uses collection_slug so NFT item page loads correctly
  function goToItem() {
  ...
  return (
    <div onClick={goToItem}
      className="group rounded-2xl overflow-hidden cursor-pointer transition-all card-hover"
      style={{ background: "#121821", border: "1px solid rgba(255,255,255,0.06)" }}>
  ...
  const fetchPortfolio = useCallback(async () => {
    ...
    try {
      const { data: nftRows, error: nftErr } = await supabase
        ...
        .ilike("owner_address", address) // case-insensitive match
    ...
    <div className="px-4 sm:px-6 py-8 max-w-7xl mx-auto fade-up">
      ...
      {!loading && ownedNFTs.length > 0 && (
        <div className="flex items-center gap-6 p-4 rounded-2xl mb-6"
          ...
          <div>
            ...
            <div className="font-mono font-bold text-lg" style={{ color: "#e6edf3" }}>{new Set(ownedNFTs.map(n => n.contract_address)).size}</div>
          </div>
          {metaLoading && (
      ...
      {loading ? (
        ...
        <div className="py-24 text-center rounded-3xl" style={{ border: "1px dashed rgba(255,255,255,0.06)" }}>
          ...
          <div className="font-bold text-lg mb-2" style={{ color: "#e6edf3" }}>No NFTs found</div>
          <p className="text-sm" style={{ color: "#9da7b3" }}>No NFTs from supported collections found in this wallet.</p>
        </div>
      )}
