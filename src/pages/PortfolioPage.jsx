...
onMouseEnter={e => e.currentTarget.style.borderColor = “rgba(34,211,238,0.4)”}
onMouseLeave={e => e.currentTarget.style.borderColor = isListed ? “rgba(34,211,238,0.25)” : “rgba(255,255,255,0.06)”}>
  {isListed && (<div className="absolute top-2 left-2 z-10 px-2 py-0.5 rounded-lg text-[9px] font-bold"
...
try {
  ...
  // Build listing map — deduplicate, keep highest listing_id per token
  const listingMap = {};
  (listingRows || [])
  ...
  // 4. ✅ KEY FIX: Also fetch NFTs currently held by the marketplace contract
  // When an NFT is listed, ownership transfers to the marketplace address,
  // so it won't appear in the wallet query above — we need to fetch it separately
  ...
  // 5. Combine wallet NFTs + listed NFTs (deduplicate by contract:tokenId)
  ...
  // 6. Merge everything into enriched NFT objects
...
// Realtime — any listing change triggers a portfolio refresh
useEffect(() => {
if (!address) return;
    ...
    <div className="flex items-center gap-1.5 mb-5 flex-wrap">
      {TABS.map(t => (
        <button key={t} onClick={() => setTab(t)}
          ...
          {t}
          {t === "Listed"   && listedCount   > 0 && ` (${listedCount})`}{t === "Unlisted" && unlistedCount  > 0 && ` (${unlistedCount})`}
        </button>
      ))}
      {metaLoading && (
