...
// price in DB = raw 6-decimal units -> always divide by 1e6 for display
...
function NFTCard({ nft, view, onList, onDelist }) {
  ...
  if (view === "list") {
    return (
      <div
        ...
        style={{
          background: "#121821",
          border: isListed ? "1px solid rgba(34,211,238,0.25)" : "1px solid rgba(255,255,255,0.06)",
        }}
        onMouseEnter={e => (e.currentTarget.style.borderColor = "rgba(34,211,238,0.4)")}
        onMouseLeave={e =>
          (e.currentTarget.style.borderColor = isListed
      ...
      >
        ...
        {isListed ? (
          <button
            onClick={e => { e.stopPropagation(); onDelist(nft); }}
  ...
  return (
    ...
    >
      {isListed && (
        <div
          className="absolute top-2 left-2 z-10 px-2 py-0.5 rounded-lg text-[9px] font-bold"
          style={{
      ...
      <div className="p-3">
        ...
        <div className="font-bold text-sm truncate mb-1" style={{ color: "#e6edf3" }}>
          {tokenName}
        </div>
...
export default function PortfolioPage() {
  ...
  const fetchPortfolio = useCallback(async () => {
    ...
    try {
      ...
      // Deduplicate — keep highest listing_id per token
      const listingMap = {};
      (listingRows || [])
      ...
      // 4. NFTs held by marketplace (listed NFTs transfer to marketplace in some contracts)
      ...
      // Only include marketplace-held NFTs that belong to THIS seller
      ...
      // 5. Merge wallet NFTs + marketplace-held NFTs (deduplicate)
      const allNftRows = [...(nftRows || [])];
      marketplaceNfts.forEach(mn => {
        const exists = allNftRows.some(
          n =>
      ...
      if (!allNftRows.length) {
        setOwnedNFTs([]);
        setLoading(false);
        return;
      }

      // 6. Enrich each NFT with collection + listing data
      ...
      // 7. Stream metadata for NFTs without images
      ...
      for (let i = 0; i < needsMeta.length; i += 8) {
        await Promise.all(
          needsMeta.slice(i, i + 8).map(async nft => {
            try {
  ...
  // Realtime — re-fetch whenever any listing for this seller changes
  useEffect(() => {
    if (!address) return;
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter(n => {
        const name = n.metadata?.name || n.name || n.collection_name || "";
        return name.toLowerCase().includes(q) || String(n.token_id).includes(q);
      });
    }
    return list;
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 fade-up">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-extrabold mb-1" style={{ color: "#e6edf3" }}>
            Connect Wallet
          </h1>
          <p className="text-sm" style={{ color: "#9da7b3" }}>
            Connect to view your NFTs on Tempo Chain.
          </p>
        </div>
      </div>
    );
    ...
    <div className="px-4 sm:px-6 py-8 max-w-7xl mx-auto fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold uppercase" style={{ color: "#e6edf3" }}>
            My NFTs
          </h1>
          <p className="mt-1 text-xs font-mono truncate max-w-xs" style={{ color: "#9da7b3" }}>
            {address}
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            ...
            >
              ...
              <div className="font-mono font-bold text-lg" style={{ color: color || "#e6edf3" }}>
                {value}
              </div>
            </div>
          ))}
      ...
      {!loading && ownedNFTs.length > 0 && (
        <div className="flex items-center gap-1.5 mb-5 flex-wrap">
          ...
          {metaLoading && (
            <span className="flex items-center gap-1.5 text-xs" style={{ color: "#9da7b3" }}>
        ...
        <div className={view === "grid" ? "grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4" : "space-y-2"}>
          {filtered.map((nft, idx) => (
            <NFTCard
              ...
              nft={nft}
              view={view}
              onList={n => setListModal({
                tokenId:    n.token_id,
                contract:   n.contract_address,
                name:       n.metadata?.name  || n.name  || ("#" + n.token_id),
                image:      n.metadata?.image || n.image || null,
                attributes: n.metadata?.attributes || [],
                collection: n.collection_name,
                slug:       n.collection_slug,
              })}
              onDelist={n => setDelistModal({
                tokenId:   n.token_id,
                contract:  n.contract_address,
                listingId: n.listing.listing_id,
                name:      n.metadata?.name  || n.name  || ("#" + n.token_id),
                image:     n.metadata?.image || n.image || null,
