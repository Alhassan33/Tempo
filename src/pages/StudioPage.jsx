 ...
 *
 * FIXES:
 *   1. Never returns null/blank — always shows a screen
 *   2. Shows the creator's existing collections when wallet is connected
 *   3. Deploy flow is wrapped in try/catch with proper state cleanup
 *   4. No race condition on wallet switch — guarded by stable address ref
 */
...
const FACTORY_ABI = [
  {
    name: "createCollection", type: "function", stateMutability: "nonpayable",
    inputs: [
  ...
  {
    name: "isApprovedCreator", type: "function", stateMutability: "view",
    inputs: [{ name: "creator", type: "address" }],
    outputs: [{ name: "", type: "bool" }],
  },
  {
    name: "openToAll", type: "function", stateMutability: "view",
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-1.5">
        <label className="text-xs font-bold uppercase tracking-wide" style={{ color: "#9da7b3" }}>
...
function LogoUpload({ logoUrl, setLogoUrl }) {
  ...
  return (
    <label className="relative flex flex-col items-center justify-center w-28 h-28 rounded-2xl cursor-pointer overflow-hidden"
...
// Uses useCreatorCollections hook from useSupabase — checks both
// creator_wallet and creator_address columns, merges live stats.
function MyCollections({ address, onManage }) {
  const { collections, isLoading: loading } = useCreatorCollections(address);

  if (loading) return (
    <div className="grid grid-cols-2 gap-3 mt-4">
      {[1, 2].map(i => (
        <div key={i} className="h-24 rounded-2xl animate-pulse" style={{ background: "#161d28" }} />
      ))}
    </div>
  );
  ...
  if (!collections.length) return (
    <div className="mt-4 rounded-2xl p-6 text-center"
      style={{ background: "#121821", border: "1px dashed rgba(255,255,255,0.08)" }}>
      <p className="text-sm" style={{ color: "#9da7b3" }}>
        You haven't deployed any collections yet. Click <strong style={{ color: "#22d3ee" }}>Create Collection</strong> below to get started.
      </p>
    </div>
  );
      ...
      {collections.map(col => (
        <div key={col.id}
          ...
          <div className="w-12 h-12 rounded-xl overflow-hidden flex-shrink-0"
            ...
            {col.logo_url
              ? <img src={col.logo_url} alt="" className="w-full h-full object-cover" />
              : <div className="w-full h-full flex items-center justify-center font-bold text-sm"
                  style={{ color: "#22d3ee" }}>{col.name?.slice(0, 2).toUpperCase()}</div>
            }
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-bold text-sm truncate" style={{ color: "#e6edf3" }}>{col.name}</div>
            <div className="text-[10px] font-mono truncate" style={{ color: "#9da7b3" }}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className="text-[10px] px-2 py-0.5 rounded-lg font-bold capitalize"
              style={{
                background: col.status === "live" ? "rgba(34,197,94,0.1)" : col.status === "draft" ? "rgba(255,255,255,0.06)" : "rgba(34,211,238,0.1)",
                color:      col.status === "live" ? "#22C55E" : col.status === "draft" ? "#9da7b3" : "#22d3ee",
              }}>
              {col.status || "draft"}
            </span>
...
export default function StudioPage() {
  ...
  const [form, setForm] = useState({
    name: "", symbol: "", description: "", max_supply: "2000",
    pre_reveal: "", website: "", twitter: "", discord: "",
  });
  ...
  function resetCreate() {
    ...
    setTxHash("");
    setNewContract("");
    setForm({ name: "", symbol: "", description: "", max_supply: "2000", pre_reveal: "", website: "", twitter: "", discord: "" });
    setLogoUrl("");
  }

  const deploy = useCallback(async () => {
    const currentAddress = addressRef.current;
    if (!publicClient || !currentAddress) return setError("Wallet not connected.");
    ...
    try {
      ...
      const [openToAll, isApproved] = await Promise.all([
        ...
        publicClient.readContract({ address: LAUNCHPAD_FACTORY, abi: FACTORY_ABI, functionName: "isApprovedCreator", args: [currentAddress] }),
      ]);
      ...
      if (!contractAddr) {
        setError("Deployed but couldn't detect address. Check: " + EXPLORER_BASE + "/tx/" + hash);
        setDeploying(false);
        return;
      }
      ...
      await Promise.all([
        ...
        supabase.from("collections").upsert({
          ...
          creator_name:      currentAddress.slice(0, 6) + "…" + currentAddress.slice(-4),
      ...
      setError(e?.shortMessage || e?.message?.slice(0, 120) || "Deployment failed");
      setDeploying(false);
      setStep(1);
    }
  }, [form, logoUrl, publicClient, writeContractAsync, wrongNetwork]);
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-bold uppercase tracking-widest"
              style={{ color: "#22d3ee", fontFamily: "Syne, sans-serif" }}>Creator Studio</span>
          </div>
        ...
        {!isConnected && (
          <div className="text-center py-16">
            ...
            <h2 className="text-xl font-extrabold mb-2" style={{ color: "#e6edf3", fontFamily: "Syne, sans-serif" }}>
              Connect Wallet
            </h2>
            <p className="text-sm" style={{ color: "#9da7b3" }}>
        ...
        {isConnected && mode === "home" && (
          <div>
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-sm font-bold uppercase tracking-widest" style={{ color: "#9da7b3" }}>
                Your Collections
              </h2>
            </div>
            <MyCollections
            ...
            <button
              onClick={() => { setMode("create"); setStep(0); setError(""); }}
              ...
              style={{ background: "#22d3ee", color: "#0b0f14", border: "none", cursor: "pointer",
                boxShadow: "0 0 32px rgba(34,211,238,0.15)", fontFamily: "Syne, sans-serif" }}>
              ...
              <div className="space-y-6">
                <div className="flex gap-5 items-start">
                  ...
                  <div className="flex-1 space-y-4">
                    <Field label="Collection Name" required>
                      <Input value={form.name} onChange={v => set("name", v)} placeholder="e.g. Tempo Cats" />
                    </Field>
                    <Field label="Token Symbol" required hint="3–8 chars">
                      <Input value={form.symbol} onChange={v => set("symbol", v.toUpperCase())} placeholder="e.g. TCAT" />
                    </Field>
                  </div>
                </div>
                ...
                <Field label="Description">
                  <textarea value={form.description} onChange={e => set("description", e.target.value)}
                    placeholder="Tell collectors what makes this special…" rows={3}
                ...
                <Field label="Pre-reveal URI" hint="Optional placeholder URI before reveal">
                  <Input value={form.pre_reveal} onChange={v => set("pre_reveal", v)} placeholder="ipfs://… (optional)" />
                </Field>
                ...
                <div className="rounded-2xl p-4" style={{ background: "rgba(34,211,238,0.04)", border: "1px solid rgba(34,211,238,0.1)" }}>
                  <div className="flex items-center gap-2 mb-1">
                    <Zap size={13} style={{ color: "#22d3ee" }} />
                    <span className="text-xs font-bold uppercase tracking-wide" style={{ color: "#22d3ee" }}>$0.05 Platform Fee</span>
                  </div>
                  <p className="text-xs" style={{ color: "#9da7b3" }}>
                    Automatically baked into every mint by the factory contract. You can't remove it and you don't need to set it up.
                  </p>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={goReview} disabled={!canProceed()}
                    ...
                    style={{
                      ...
                      boxShadow: canProceed() ? "0 0 24px rgba(34,211,238,0.2)" : "none",
              ...
              <div className="space-y-5">
                <div className="rounded-2xl overflow-hidden" style={{ background: "#121821", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="flex items-center gap-4 p-5 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                    <div className="w-16 h-16 rounded-2xl overflow-hidden flex-shrink-0" style={{ background: "#161d28" }}>
                      {logoUrl
              ...
              <div className="text-center space-y-6 py-8">
                {!deployed ? (
                  <>
                    ...
                    {error && (
                      <div className="flex items-center gap-2 rounded-xl px-4 py-3 text-xs mx-auto max-w-sm"
                        style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", color: "#EF4444" }}>
                        <AlertCircle size={13} /> {error}
                      </div>
                    )}
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-sm mb-1" style={{ color: "#9da7b3" }}>
                        Contract: <code className="font-mono text-xs" style={{ color: "#22d3ee" }}>{newContract}</code>
                      </p>
                      <p className="text-sm" style={{ color: "#9da7b3" }}>
                    ...
                    <div className="flex gap-3 justify-center">
                      ...
                      <button onClick={() => navigate(`/studio/manage/${newContract}`)}
