...
import { useCollection, useListings, useRealtimeListings } from "@/hooks/useSupabase";
...
import { extractImageUrl, resolveIpfsUrl } from "@/utils/nftImageUtils.js";
...
// ─── NFT Grid Item ────────────────────────────────────────────────────────────
function NFTCard({ token, listing, onBuy, onList, isOwner, view }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-sm rounded-2xl p-6"
        ...
        {error && (
          <div className="rounded-xl px-4 py-2 mb-4 text-xs" style={{ background: "rgba(239,68,68,0.1)", color: "#EF4444", border: "1px solid rgba(239,68,68,0.3)" }}>
            {error}
          </div>
        )}
...
export default function CollectionPage() {
  ...
  const { listings } = useRealtimeListings(
    collection?.contract_address || NFT_CONTRACT
  );
  ...
  // Load tokens from IPFS metadata
  useEffect(() => {
  ...
  // Build listing map for quick lookup
  ...
  return (
    <div className="fade-up">
      ...
      <div className="relative h-44 w-full overflow-hidden"
        ...
        {col.banner_url && (
          <img src={col.banner_url} alt="" className="w-full h-full object-cover" />
        )}
      ...
      <div className="px-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-end gap-5 -mt-10 mb-6 relative z-10 flex-wrap">
          ...
          <div className="pb-1 flex-1 min-w-0">
            ...
            <p className="text-sm mt-0.5" style={{ color: "#9da7b3" }}>
              {col.total_supply?.toLocaleString()} items
            </p>
            {col.description && (
              <p className="text-xs mt-1 max-w-md" style={{ color: "#9da7b3" }}>{col.description}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 pb-1">
            ...
            <a
              href={`https://explorer.moderato.tempo.xyz/address/${col.contract_address}`}
              ...
              {tokensLoading ? (
                ...
                <>
                  ...
                  <div className="flex items-center justify-center gap-3 mt-8">
                    ...
                    <span className="text-sm font-mono" style={{ color: "#9da7b3" }}>
                      {page} / {totalPages}
                    </span>
                    <button
