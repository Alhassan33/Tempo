...
// Minimal ERC-721 ABI for enrichment
const ERC721_ABI = [
...
async function enrichCollection(contractAddress) {
  ...
  try {
    ...
    // 3. Fetch token #1 metadata for logo/description
    let logoUrl = null;
    let bannerUrl = null;
      ...
      try {
        ...
        if (httpUri) {
          ...
          if (metadata) {
            ...
            // Some collections store banner in animation_url or other fields
            bannerUrl = ipfsToHttp(metadata.banner_image || metadata.animation_url) || null;
          }
        }
      } catch (e) {
    ...
    const { error } = await supabase.from("collections").insert({
      ...
      name: name.slice(0, 100), // safety limit
      ...
      total_minted: Number(totalSupply), // assume all minted if totalSupply works
      verified: false,
      floor_price: 0,
      volume_total: 0,
      volume_24h: 0,
      ...
      creator_name: creatorWallet 
        ...
        : "Unknown",
      metadata_base_uri: "", // can be derived later
    });
